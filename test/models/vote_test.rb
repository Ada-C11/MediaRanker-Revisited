...
describe Vote do
  ...
  describe "validations" do
    let (:user1) { User.create!(username: "chris") }
    let (:user2) { User.create!(username: "dee") }
    ...
    it "allows one user to vote for multiple works" do
      vote1 = Vote.new(user_id: user1, work: work1)
      vote1.save!
      vote2 = Vote.new(user_id: user1, work: work2)
      vote2.valid?.must_equal true
    end

    it "allows multiple users to vote for a work" do
      vote1 = Vote.new(user_id: user1.id, work_id: work1)
      vote1.save!
      vote2 = Vote.new(user_id: user2.id, work_id: work1)
      vote2.valid?.must_equal true
    end

    it "doesn't allow the same user to vote for the same work twice" do
      vote1 = Vote.new(user_id: user1, work_id: work1)
      vote1.save!
      vote2 = Vote.new(user_id: user1, work_id: work1)
