...
describe Work do
  ...
  describe "vote_count" do
    ...
    it "tracks the number of votes" do
      work = Work.create!(title: "test title", category: "movie")
      4.times do |i|
        user = User.create!(uid: User.last.uid + 1, provider: "test", username: User.last.username + "a", email: "test.com")
  ...
  describe "top_ten" do
    before do
      ...
      test_users = []
      20.times do |i|
        test_users << User.create!(username: "user#{i}", uid: User.last.uid + 1, provider: "github")
      end
